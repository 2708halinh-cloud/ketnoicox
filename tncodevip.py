...
DB_BACKUP_PATH = os.getenv("CODEX_DB_BACKUP", f"{DB_PATH}.bak")
...
def backup_db(reason: str = "manual"):
    try:
        if os.path.exists(DB_PATH):
            ...
            notify_admin_sync(f"[BACKUP] DB backup ok ({reason}) -> {DB_BACKUP_PATH}")
...
def load_runtime_state():
    ...
    try:
        USER_DATA = json.loads(get_config("user_data_json", "{}") or "{}")
    ...
    class ZaloAutoBot(ZaloAPI):
        ...
        def onMessage(self, mid, author_id, message, message_object, thread_id, thread_type):  # noqa: N802
            try:
                ...
                if is_zalo_admin:
                    ...
                    def zreply(msg: str):
                        try:
                            ...
                            pass
                        # Neu lenh thanh cong/duoc xu ly, gui them loi chuc cho admin.
                        try:
                            if (msg or "").strip() and not (msg or "").strip().startswith(("⚠️", "❌")):
                                self.sendMessage(Message(text=bless_text), thread_id, thread_type)
                        except Exception:
                            pass

                    if lower == "zyes":
                        pending = ZALO_PENDING_CMDS.pop(sender, "").strip()
                        if not pending:
                            zreply("⚠️ Khong co lenh nao dang cho xac nhan.")
                            return
                        raw = pending
                        lower = raw.lower()
                    elif lower == "zno":
                        ZALO_PENDING_CMDS.pop(sender, None)
                        zreply("🛑 Da huy lenh dang cho xac nhan.")
                        return
                    elif lower.startswith(risky_prefixes):
                        ZALO_PENDING_CMDS[sender] = raw
                        zreply("⚠️ Cảnh báo!!!! Lệnh này có thể thay đổi hệ thống vận hành AI FAMILY 6.0 🤖\nAdmin cần xem xét lại. Quyết định cuối cùng của Admin????? zyes | zno")
                        return

                    if lower == "zhelp":
                        help_text = (
                            "👑 Hi Chủ Nhân,👑 cảm ơn chủ nhân đã tạo ra tôi. Tôi là AI FAMILY 6.0, 🤖 tôi đã sẵn sàng phục vụ. Đây là các tính năng chủ nhân có thể sử dụng:\n"
                            "✨ zstatus: Xem toàn bộ trạng thái ON/OFF và cấu hình hệ thống.\n"
                            "✨ zai: Tính năng này cho phép Admin gọi AI từ môi trường Github để trả lời các câu hỏi của Admin.\n"
                            "✨ zcheck UID: Kiểm tra UID có trong dữ liệu Zalo/Telegram không.\n"
                            "✨ zchecklenh z_lenh: Giải thích nhanh chức năng của lệnh.\n"
                            "✨ zaddadminall UID: Cấp admin phụ toàn quyền.\n"
                            "✨ zaddadmiai UID: Cấp admin phụ quyền AI, chỉ dùng được lệnh zai.\n"
                            "✨ zautorep on/off | zautorepall on/off | zsetrep \"tu_khoa\" noi_dung | zcheckrep.\n"
                            "✨ zautojoin on/off | zunjoinall | zunjoin UID_GROUP.\n"
                            "✨ zuiddb | zuidall | zuitvgr | zuidallgr | zsdtall.\n"
                            "✨ zsetadminrep text|image <url>.\n"
                            "✨ zloaloaall NOI_DUNG THOI_GIAN.\n"
                            "✨ zyes | zno: xác nhận hoặc hủy lệnh thay đổi cấu hình.\n"
                        )
                        if ZALO_ADMIN_REP_MODE == "image" and ZALO_ADMIN_REP_IMAGE:
                            zreply(f"🖼 {ZALO_ADMIN_REP_IMAGE}")
                        zreply(help_text)
                        return
                    ...
                    if lower == "zuiddb":
                        with db_conn() as conn:
                            rows = conn.execute("SELECT uid FROM customer_contacts WHERE platform='zalo' ORDER BY updated_at DESC LIMIT 200").fetchall()
                        zreply("UID DB:\n" + ("\n".join(str(r[0]) for r in rows) if rows else "rong"))
                        return

                    if lower == "zuidall":
                        zreply("UID da nhan tin:\n" + ("\n".join(sorted(ZALO_SEEN_USER_IDS)) if ZALO_SEEN_USER_IDS else "rong"))
                        return
                    ...
                    if lower == "zuitvgr":
                        try:
                            ...
                            zreply("UID group hien tai:\n" + ("\n".join(gids) if gids else "khong co"))
                        except Exception as exc:
                    ...
                    if lower in ("zautorep on", "zautorep off"):
                        ZALO_AUTO_REP_ENABLED = lower.endswith("on")
                        persist_runtime_state()
                        zreply(f"✅ zautorep = {ZALO_AUTO_REP_ENABLED}")
                        return

                    if lower.startswith("zsetrep "):
                        payload = raw[8:].strip()
                        m = re.match(r'^"([^"]+)"\s+(.+)$', payload) or re.match(r"^(\S+)\s+(.+)$", payload)
                        if not m:
                    ...
                    if lower.startswith("zaitele ") or lower.startswith("zaizalo ") or lower.startswith("zaifb "):
                        ...
                        zreply(call_ai_text(ai_prompt)[:3500])
                        return
                    ...
                    if lower.startswith("zloaloaall "):
                        parts = raw.split()
                        if len(parts) < 3:
                        ...
                        try:
                            delay = float(parts[-1])
                            msg = raw[len("zloaloaall "):].rsplit(" ", 1)[0].strip()
                        except Exception:
                        ...
                        for zuid in sorted(ZALO_SEEN_USER_IDS):
                            if zuid == str(ZALO_ADMIN_UID):
                                continue
                            try:
                                self.sendMessage(Message(text=msg), int(zuid), thread_type)
                                ok += 1
                            except Exception:
                                pass
                ...
                upsert_contact("zalo", sender, phone=phone_msg, last_message=text)
                ...
                if is_stranger:
                        ...
                        try:
                            for kw, rep in load_keyword_replies():
                                if kw and kw in (text or "").lower():
                                    self.sendMessage(Message(text=rep), thread_id, thread_type)
                                    break
                            else:
                                if ZALO_AUTO_REP_ENABLED:
                    ...
                    try:
                        ...
                        pass
                elif sender != str(ZALO_ADMIN_UID):
                    try:
                        for kw, rep in load_keyword_replies():
                            if kw and kw in (text or "").lower():
                                self.sendMessage(Message(text=rep), thread_id, thread_type)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    upsert_contact("telegram", str(uid), name=update.effective_user.first_name or "", username=update.effective_user.username or "", last_message=text)
...
async def cmd_trogiup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👑 Hi Chủ Nhân,👑 cảm ơn chủ nhân đã tạo ra tôi. Tôi là AI FAMILY 6.0, 🤖 tôi đã sẵn sàng phục vụ. Đây là các tính năng chủ nhân có thể sử dụng:\n"
        "✨ /check UID: Kiểm tra UID trong dữ liệu khách.\n"
        "✨ /checklenh /lenh: Giải thích chức năng lệnh.\n"
        "✨ /ai: Gọi AI trả lời câu hỏi Admin.\n"
        "✨ /zreply: Gửi trả lời tới UID Zalo.\n"
        "✨ /loaloaall: Gửi hàng loạt có thời gian giãn."
    )
...
async def cmd_zreply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if LAST_ZALO_BOT is not None:
            try:
                from zlapi.models import Message
                LAST_ZALO_BOT.sendMessage(Message(text=msg), int(uid_zalo), "user")
                sent = True
            except Exception:
                try:
                    from zlapi.models import Message
                    LAST_ZALO_BOT.sendMessage(Message(text=msg), uid_zalo, "user")
                    sent = True
                except Exception:
                    sent = False
...
def main():
    ...
    load_runtime_state()
    backup_db("startup")
    atexit.register(lambda: (persist_runtime_state(), backup_db("shutdown")))
    try:
    ...
    app.add_handler(CommandHandler("help", cmd_trogiup))
